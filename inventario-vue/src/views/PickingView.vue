...
const fmtFecha = (f) => new Date(f + 'T00:00:00').toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' });
...
// En Picking solo se ven las salidas AÚN no enviadas (de cualquier día), agrupadas por día.
const pendientes = computed(() => candidatas.value.filter(r => !enviadoPorKey.value[r.key]));
const pendientesPorDia = computed(() => {
  const map = {};
  for (const r of pendientes.value) (map[r.fecha] ||= []).push(r);
  return Object.keys(map).sort().map(fecha => ({ fecha, refs: map[fecha] }));
});
...
// --- Helpers POR DÍA (el picking se envía por día) ---
const refsDeDia = (fecha) => pendientes.value.filter(r => r.fecha === fecha);
const numListasDia = (fecha) => refsDeDia(fecha).filter(completa).length;
const diaCompleto = (fecha) => { const rs = refsDeDia(fecha); return rs.length > 0 && rs.every(completa); };
const totalPalletsDia = (fecha) => refsDeDia(fecha).reduce((s, r) => s + r.pallets, 0);
// Un HU no puede repetirse dentro del mismo día
function huExisteEnDia(fecha, code) {
  return refsDeDia(fecha).some(r => (estado.value[r.key]?.hus || []).some(h => (h || '').trim() === code));
}
function dupSetDia(fecha) {
  const cont = {};
  for (const r of refsDeDia(fecha)) {
...
const hayDuplicados = computed(() => pendientesPorDia.value.some(d => dupSetDia(d.fecha).size > 0));
...
function siguiente(r) {
  ...
  const sig = refsDeDia(r.fecha).find(x => !completa(x));
...
function onEscaneo(code) {
  ...
  const c = (code || '').trim();
  if (!c) return;
  if (huExisteEnDia(r.fecha, c)) { showInfo('Ese HU ya está escaneado (en esta o en otra referencia del día).'); return; }
...
// --- Envío POR DÍA ---
const enviandoDia = ref(null);
async function enviarDia(fecha) {
  if (dupSetDia(fecha).size) { showError('Hay HU repetidos: ' + [...dupSetDia(fecha)].join(', ')); return; }
  if (!diaCompleto(fecha)) { showError('Faltan referencias por completar.'); return; }
  enviandoDia.value = fecha;
  try {
    const pend = refsDeDia(fecha);
    ...
    const payload = {
      ...
      total_pallets: totalPalletsDia(fecha),
...
<template>
  <div class="max-w-3xl mx-auto">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
      ...
      Si un material no lleva HU, pulsa <strong>"No tiene HU"</strong>. <strong>Cada día se envía por separado.</strong>
    </p>
    ...
    <!-- Un bloque por cada día pendiente -->
    <div v-else v-for="dia in pendientesPorDia" :key="dia.fecha" class="mb-10">
      <div class="flex items-center justify-between gap-3 mb-3">
        <h2 class="text-lg font-bold text-gray-800 dark:text-white capitalize">Salida del {{ fmtFecha(dia.fecha) }}</h2>
        <span class="text-sm font-semibold shrink-0" :class="diaCompleto(dia.fecha) ? 'text-brandgreen-600' : 'text-brand-600'">
          {{ numListasDia(dia.fecha) }} / {{ dia.refs.length }} · {{ totalPalletsDia(dia.fecha) }} pallets
        </span>
      </div>
          ...
          <div v-if="!estado[r.key]?.noHU" class="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div v-for="(hu, i) in estado[r.key].hus" :key="i" class="flex items-center gap-2">
              ...
              <input
                ...
                :class="(hu || '').trim() && dupSetDia(r.fecha).has((hu || '').trim()) ? 'border-red-500 bg-red-50 dark:bg-red-900/30' : ((hu || '').trim() ? 'border-brandgreen-400 bg-brandgreen-50 dark:bg-gray-700' : 'border-gray-300')"
              />
      ...
      <!-- Aviso de HU repetidos de ese día -->
      <div v-if="dupSetDia(dia.fecha).size" class="mt-3 flex items-start gap-2 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 text-red-700 dark:text-red-300 text-sm font-semibold px-4 py-2.5 rounded-lg">
        <NoSymbolIcon class="w-5 h-5 shrink-0 mt-0.5" />
        <span>HU repetido, revísalo antes de enviar: <span class="font-mono">{{ [...dupSetDia(dia.fecha)].join(', ') }}</span></span>
      </div>

      <!-- Botón enviar de ESE día -->
      <button
        @click="enviarDia(dia.fecha)"
        :disabled="enviandoDia === dia.fecha || !diaCompleto(dia.fecha) || dupSetDia(dia.fecha).size > 0"
        class="mt-3 w-full flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition-colors"
      >
        <PaperAirplaneIcon class="w-5 h-5" />
        {{ enviandoDia === dia.fecha ? 'Enviando…' : (dupSetDia(dia.fecha).size ? 'HU repetido' : (diaCompleto(dia.fecha) ? `Enviar salida del ${fmtDia(dia.fecha)}` : `Faltan ${dia.refs.length - numListasDia(dia.fecha)} referencias`)) }}
      </button>
    </div>
